...
pinMode(LED, OUTPUT);
    ...
    tone (BIP, 2000);
    delay(250);//Delay apagar LED Verde
    noTone(BIP);
...
if (conteudo.substring(1) == "95 0F 62 51") //ID CARTÃO
  {
    digitalWrite (LED, HIGH);
    tone (BIP, 500);
...
void dump_byte_array(byte *buffer, byte bufferSize) {
  for (byte i = 0; i < bufferSize; i++) {
    ...
    conteudo.concat(String(mfrc522[i].uid.uidByte[i] < 0x10 ? " 0" : " "));
    conteudo.concat(String(mfrc522[i].uid.uidByte[i], HEX));
  }
}
